...
import com.github.cstoku.neologd.unidic.lucene.analysis.ja.dict.ConnectionCosts;
...
public class GraphvizFormatter {
  ...
  private String formatNodes(JapaneseTokenizer tok, JapaneseTokenizer.WrappedPositionArray positions, int startPos, JapaneseTokenizer.Position endPosData, char[] fragment) {
    ...
    for (int pos = endPosData.pos; pos > startPos; pos--) {
      final JapaneseTokenizer.Position posData = positions.get(pos);
      for(int idx=0;idx<posData.count;idx++) {
        final JapaneseTokenizer.Position backPosData = positions.get(posData.backPos[idx]);
